...
Para abrir Git en el proyecto, se debe hacer clic derecho Git Bash Here en la carpeta donde se ubica dicho proyecto. Una vez abierto, con \texttt{git init} se inicia un nuevo repositorio con una rama llamada Master. Luego, se envía el archivo al repositorio con \texttt{git add nom-archivo.extension} para cada archivo, sin embargo, también se pueden añadir todos los archivos con \texttt{git add .}. \\
Lista de comandos: 
\begin{enumerate}
...
\item \texttt{git rm --cached nom-archivo.extension}: para volver un paso atrás. Si se quiere hacer con todos los archivos: \texttt{git rm --cached .}.
\item \texttt{git clone direccion-del-repositorio}: clonar un repositorio.
\item \texttt{clear}: limpiar la pantalla.
\end{enumerate}
Para que git ignore ciertos archivos, se debe crear un archivo llamado .gitignore que contenga las extensiones de los proyectos (Ejemplo: *.mp4) o el nombre de este (Ejemplo: proy1.txt).
%	SUBSECCION
\subsection{Ramas}
