...
\lstset{
	...
	breaklines=true,            % Activar saltos de línea
	breakatwhitespace=false,    % Romper líneas incluso sin espacios
	prebreak=\textbackslash,    % Indicar línea rota
	postbreak=\space,           % Espacio tras salto
	breakindent=10pt,           % Indentación tras quiebre
	frame=single,               % Cuadro alrededor del código
	numbers=left,               % Numeración de líneas
	numberstyle=\tiny,          % Estilo para los números
	showspaces=false,			% No muestra los espacios
	tabsize=4,					% Numero de espacios de la tabulacion
	prebreak=\textellipsis		% Indicador de corte
}
% DEFINIENDO COLORES DE LOS CODIGOS
% Definir colores para los lenguajes
\definecolor{sqlkeywords}{RGB}{0,0,255} % Azul para palabras clave de SQL
\definecolor{sqlcomments}{RGB}{0,128,0} % Verde para comentarios de SQL
\definecolor{sqlstrings}{RGB}{163,21,21} % Rojo oscuro para cadenas de texto en SQL

\definecolor{pythonkeywords}{RGB}{204,120,50} % Naranja para palabras clave de Python
\definecolor{pythoncomments}{RGB}{0,128,0} % Verde para comentarios de Python
\definecolor{pythonstrings}{RGB}{163,21,21} % Rojo oscuro para cadenas de texto en Python

% Estilo para SQL
\lstdefinestyle{sqlstyle}{
	language=SQL,
	keywordstyle=\color{sqlkeywords}\bfseries,
	commentstyle=\color{sqlcomments}\itshape,
	stringstyle=\color{sqlstrings},
	basicstyle=\ttfamily\small,
	morekeywords={ON, USING, DATABASE, SCHEMA, IF, EXISTS}, % Palabras clave adicionales
...
\lstdefinestyle{pythonstyle}{
	...
	commentstyle=\color{pythoncomments}\itshape,
	stringstyle=\color{pythonstrings},
	basicstyle=\ttfamily\small,
	breaklines=true % las lineas de codigo se rompen si son muy largas      
}

% Definir colores para R
\definecolor{rkeywords}{RGB}{86,156,214} % Azul para palabras clave
\definecolor{rcomments}{RGB}{0,128,0}    % Verde para comentarios
\definecolor{rstrings}{RGB}{163,21,21}   % Rojo oscuro para cadenas
\definecolor{rnumbers}{RGB}{128,0,128}   % Morado para números

% Estilo para R
\lstdefinestyle{rstyle}{
	language=R,
	keywordstyle=\color{rkeywords}\bfseries,
	commentstyle=\color{rcomments}\itshape,
	stringstyle=\color{rstrings},
	basicstyle=\ttfamily\small,
	...
	numbersep=5pt,           % Espaciado entre los números y el código
	frame=single,            % Cuadro alrededor del código
	rulecolor=\color{black}, % Color del borde del cuadro
	breaklines=true,         % Permitir saltos de línea
	breakatwhitespace=false, % Permitir saltos sin depender de espacios
...
En las consultas SQL, la cláusula \textcolor{blue}{WHERE} filtra los datos según la condición que se le otorgue. Se pueden ordenar los datos en orden ascendente (\textcolor{blue}{ASC}) o descendente (\textcolor{blue}{DESC}). Se le agrega un \textcolor{blue}{LIKE} para buscar un patrón en específico, el cual se usa con $\%$. Este símbolo se usa para buscar los registros que empiecen con una cadena de texto:
\begin{lstlisting}[style=sqlstyle]
SELECT col-sep-por-coma
FROM conj-de-datos.tabla
WHERE cliente LIKE 'r%';
\end{lstlisting}
donde devuelve todos los clientes que comiencen con la letra r.

También se puede usar el operador BETWEEN, el cual filtra por números y fechas dentro de un rango.
\begin{lstlisting}[style=sqlstyle]
SELECT columnas
FROM conj-de-datos.tabla
WHERE fechas BETWEEN '2002-01-01' AND '2003-01-01';
\end{lstlisting}
%	SUBSECCION
...
\section{Orden}
\begin{lstlisting}[style=rstyle]
# Se usa para elegir que variable se quiere ordenar.
arrange()
...
# Permite fusionar columnas entre si.
unite(df, 'nom-col', col-a-combinar, col-a-combinar-2, sep='separador')
# Se puede usar para agregar columnas con calculos.
mutate()
\end{lstlisting}
%	SECCION
...
\begin{lstlisting}[style=rstyle]
ggplot((data=df) + geom_funcion(mapping=aes(x=var-x, y=var-y, color=var1, shape=var2, size=var3, alpha=var4))), (color= color-para-todos-los-puntos) + labs() + annotate(text)
\end{lstlisting}
...
# Dataset.
data
# Usa puntos para crear diagramas (figura geometrica).
geom_funcion()
# Define como se aplican las variables a las propiedades visuales.
mapping # Va junto a la funcion aes()
# Especifica las variables a aplicar a los ejes x e y, como el color (color),
# color de relleno (fill), forma de puntos (shape), tipo de linea (linetype)
# y tamanio (size).
aes()
...
labs
# Se utiliza para agregar texto dentro de la cuadricula.
anotate(text)
# Mas informacion en https://ggplot2.tidyverse.org/reference/annotate.html
\end{lstlisting}
...
# Grafico de lineas.
geom_line()
# Suavizado. Crea una linea de tendencia.
geom_smooth()
# Crea un histograma.
geom_histogram()
# Crea un diagrama de dispersion y agrega un poco de ruido aleatorio para lidiar con la superposicion de puntos.
geom_jitter()
\end{lstlisting}
...
Las técnicas de escalado de datos más usadas son Standard scaler y Min Max.

Standard scaler se usa para escalar características a una media de 0 y una desviación estándar de 1. Mejora el rendimiento del modelo dado que previene que las características con valores más grandes dominen por sobre otras.

\begin{lstlisting}[style=pythonstyle]
# Estandarizando las caracteristicas
scaler = StandardScaler()

variable_escalada = scaler.fit_transform(variable)
\end{lstlisting}

Por otra parte, Min Max scaler escala el conjunto de datos entre 0 (mínimo) y 1 (máximo). Se utiliza para evitar la media cero y varianza unitaria.
\begin{lstlisting}[style=pythonstyle]
# Normalizando las caracteristicas
scaler_norm = MinMaxScaler()

variable_escalada = scaler_norm.fit_transform(variable)
\end{lstlisting}

¿Cuándo normalizar y cuándo estandarizar?

La normalización es útil cuando la distribución de los datos no se conoce o no sigue una distribución gaussiana, y en algoritmos que se basan en distancias entre puntos de datos, como K-NN.

Por otra parte, la estandarización se usa en algoritmos basados en el gradiente, como SVM, o si las regresiones lineales y logísticas se pueden beneficiar de ella. También se usa como técnica de reducción de dimensionalidad.
...
\begin{lstlisting}[style=pythonstyle]
# SMOTE
#agregamos la libreria (las anteriores las supondremos cargadas
...
# Realizar clustering con diferentes valores de k y calcular las metricas
for k in k_values:
...
plt.plot(m_values, silhouette_scores_fuzzy, 'bo-', label='Puntuacion de silueta')
...
plt.xlabel('Caracteristica 1')
plt.ylabel('Caracteristica 2')
plt.title(f'Clustering jerarquico - Enlace: {method} - s-score: {sc:.2f}')
...
Este modelo se utiliza cuando los datos no se agrupan en conjuntos bien definidos. Los agrupa basándose en la densidad e identificando los datos por su proximidad.
