...
Para realizar una regresión lineal en Python, se debe dividir el conjunto de datos en dos: los datos de entrenamiento y los de prueba. Con \texttt{\textcolor{blue}{train$\_$test$\_$split()}} se dividen los datos, donde sus argumentos son:
\begin{enumerate}
...
\end{enumerate}
Luego, se crea una instancia del objeto de regresión lineal para entrenar el modelo con \texttt{\textcolor{blue}{X$\_$train}} e \texttt{\textcolor{blue}{y$\_$train}} y predecir con \texttt{\textcolor{blue}{X$\_$test}}, guardando las predicciones en \texttt{\textcolor{blue}{y$\_$pred}}. Finalmente, se evalúa el modelo mediante el error cuadrático medio y el $R^{2}$ usando \texttt{\textcolor{blue}{y$\_$test}} e \texttt{\textcolor{blue}{y$\_$pred}}.
\begin{minted}[breaklines, breakanywhere]{Python}
import numpy as np
...
\begin{enumerate}
	\item Clustering (segmentación): utiliza algoritmos para agrupar los datos en grupos basados en similitudes.
	\item Reducción de dimensionalidad: utilizan técnicas para reducir la cantidad de variables en un conjunto.
\end{enumerate}

\subsubsection{Clustering}
El uso de los custers permite identificar patrones.
...
imagenes = df.images[:20]
# Obteniendo las etiquetas
etiquetas = df.target[:20]

# Configurando el subplot
fig, axs = plt.subplots(4,5, figsize=(8,8))

# Iterando sobre las imagenes y etiquetas
for i, (imagen, etiqueta) in enumerate(zip(imagenes,etiquetas)):
	# Obteniendo coordenadas del subplot actual
	row = i // 5
	col = i % 5
	
	# Mostrando la imagen en el subplot correspondiente
	axs[row,col].imshow(imagen, cmap='gray')
	axs[row,col].set_title(f'titulo: {etiqueta}')
